...
- **Stage 4 final-synthesis sub-stage** (`final_synthesis_stage.py`) — between the work loop and the pitch sub-loop, the per-capability fragments under `executions/v{N}/work/<capability>/` are assembled into ONE coherent deliverable under `executions/v{N}/final/<bucket>/`. Project-type-aware: `algorithm` → `final/library/<package>/...`, `report` → `final/document/<slug>.md` + `.docx`, `hybrid` → both, `workflow` → `final/application/run.py` + bucketed layout. Follows the canonical preflight CLI → orchestrator fan-out → postflight CLI pattern: `final-synthesize-start` writes `runtime/final_synthesis/work_plan.yaml` + `synthesis_request.yaml`; `.github/prompts/final-synthesis.prompt.md` invokes `@final-synthesis-orchestrator` which fans out one synthesizer per modality (`@library-synthesizer` / `@document-synthesizer` / `@workflow-synthesizer`) and persists each return JSON verbatim to `subagent_returns/<modality>.json`; `final-synthesize-finalize` validates each return, runs a REQ-trace continuity check (every `REQ-NNN` mentioned in fragments must still appear under `final/`, modulo `--allow-req-drop` overrides), atomically swaps `final/.tmp/` into place, and emits `executions/v{N}/final_synthesis_report.yaml`. The `gate_final_synthesize_request` hook blocks the postflight until the work plan + request + ≥1 subagent return all exist. After synthesis, `phase4-finalize --finalize` reads `final/` first when compiling the manifest (`synthesis_status: synthesized`), the pitch evidence pack additionally exposes `assembled_deliverables[]` keyed by `ev-final-NNN`, and `@pitch-writer` cites those on the `built` slide instead of per-capability `ev-deliv-*` fragments.
...
After the Stage 1A baseline produces per-source concept pages, `/wiki-enrich` is the preferred single chat entry point for semantic enrichment. It refreshes new seed findings when needed, then runs the two dedicated passes that reconcile concepts across sources. The conductor prompt still follows the same CLI/LLM split — preflight CLI → orchestrator fan-out → CLI postflight — and the page-rewrite logic lives in CLI code, not the orchestrator prompt. Subagent JSON returns are validated against schema in `validation.py` before the CLI mutates any page; the orchestrator prompt is a thin dispatcher that persists returns to disk.
...
The `wiki_linking.py` linker indexes every canonical page's `aliases:` list as secondary display names, so a mention of "Johnson noise" anywhere in the wiki now links to `concept-thermal.md`. `/wiki-enrich` runs `meta-compiler wiki-link --version 2` at the end; when running the lower-level commands manually, run the linker after reconciliation/synthesis to pick up the new aliases.

New-seed arrival is handled by `/wiki-enrich --scope new`, which uses `wiki-update` for the low-level refresh branch and invokes the guarded ingest fan-out only when new extraction is pending. The `meta-compiler wiki-update` command remains a convenience wrapper for `ingest --scope new` + `research-breadth`: it runs the ingest preflight, halts with a remediation message if new seeds need orchestrator extraction (so `research-breadth` doesn't rebuild the index from stale findings), and otherwise refreshes the v1 wiki. Pass `--force` to refresh the index from existing findings without waiting for extraction. `track-seeds` reports the handoff but does not mutate the wiki itself.
