...
argument-hint: "What rule or convention should this enforce?"
agent: agent
---
Related skill: `agent-customization`. Load and follow [instructions.md](../skills/agent-customization/references/instructions.md) for template and principles.

Guide the user to create an instructions file.

## Extract from Conversation
First, review the conversation history. If the user has been correcting the agent's output or asking for specific patterns, generalize that into a persistent instruction. Extract:
...
Remember to follow the `agent-customization` guidance and make the description discoverable.