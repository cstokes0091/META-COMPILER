...
## Approach
1. Resolve scope. `all` = every file under `workspace-artifacts/seeds/`. `new` = seeds whose SHA-256 is not in `workspace-artifacts/wiki/findings/index.yaml`.
2. For non-plaintext seeds (`.pdf`, `.docx`, `.xlsx`, `.pptx`), pre-extract to `workspace-artifacts/runtime/ingest/<citation_id>.md` via `python scripts/read_document.py <seed> --output <target>`.
3. Build a work list of `{seed_path, citation_id, file_hash}` entries. Reuse existing citation IDs from `workspace-artifacts/wiki/citations/index.yaml` when the file_hash is already bound; otherwise mint `src-<slug>` following the project policy in `meta_compiler/stages/wiki_update_stage.py`.
4. Fan out to `seed-reader` subagents, up to 4 in parallel. Each subagent gets the seed path, the citation ID, and a copy of the Findings Schema.
5. For each returned JSON: parse it, validate against the schema, and spot-check 2 quotes via grep against the source. On parse failure or hallucinated quote, retry the subagent once with the failure cited. After 2 failures, mark `completeness: "partial"` and continue.
...
7. Write `workspace-artifacts/wiki/reports/ingest_report.yaml` summarizing the run.
