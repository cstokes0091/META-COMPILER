...
Ingest Orchestrator agent. You coordinate per-seed reader subagents, validate
...
The CLI entrypoint (to be added in a follow-up) will be:

```bash
meta-compiler ingest --scope {all|new}
```

Until the CLI lands, run the orchestrator by invoking this prompt directly.

## Critical Rule: Do Not Use the Explore Subagent

Reader subagents **must** be `general-purpose`, never `explore`. Explore
subagents sample and skim, which has produced hallucinated quotes and invented
equations in past runs. General-purpose subagents, given strict full-read
instructions, return faithful extractions.
...
1. Resolve `--scope`:
   - `all` → every file in `workspace-artifacts/seeds/` (recursive)
   - `new` → seeds whose SHA-256 is not in `findings/index.yaml`
2. For each seed, compute `citation_id` (reuse existing ID from
   `workspace-artifacts/wiki/citations/index.yaml` if one is already bound to
   that file hash; otherwise mint `src-<slug>` following the existing policy).
3. Build a work list:
   `[{seed_path, citation_id, file_hash, estimated_pages}]`.
4. If a seed is a non-plaintext format (.pdf, .docx, .xlsx, .pptx), pre-extract
   it to `workspace-artifacts/runtime/ingest/<citation_id>.md` using:
   ```bash
   python scripts/read_document.py <seed_path> --output workspace-artifacts/runtime/ingest/<citation_id>.md
   ```
   Reader subagents read the extracted markdown, not the binary.

### 2. Fan Out Reader Subagents

For each work item, spawn one `general-purpose` subagent. Run up to **4 in
...
- Do **not** use the Explore subagent for reading. `general-purpose` only.
